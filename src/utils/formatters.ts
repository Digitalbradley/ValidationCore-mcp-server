export function formatValidationForClaude(validation: any, billing?: any): string {
 ...
 */
function formatWithAgentContext(agentContext: any, validation: any, billing?: any): string {
  ...
  if (agentContext.priority_issues?.length > 0) {
    ...
    agentContext.priority_issues.forEach((issue: any, index: number) => {
  ...
  if (agentContext.suggested_actions?.length > 0) {
    ...
    agentContext.suggested_actions.forEach((action: any, index: number) => {
 ...
 */
function formatLegacy(validation: any, billing?: any): string {
  ...
  if (gmcCompliance.violations?.length > 0) {
    result += `**Issues Found:**\n`;
    gmcCompliance.violations.forEach((violation: string, index: number) => {
  ...
  if (performanceOptimization.priorityRecommendations?.length > 0) {
    ...
    performanceOptimization.priorityRecommendations.slice(0, 5).forEach((rec: string, index: number) => {
      result += `${index + 1}. ${rec}\n`;
    });
    result += '\n';
  }
...
export function formatGTINForClaude(analysis: any, billing?: any): string {
  ...
  if (!analysis.isValid && analysis.errors?.length > 0) {
    ...
    analysis.errors.forEach((error: string, index: number) => {
...
export function formatBatchForClaude(results: any[], billing?: any): string {
  ...
  if (hasAgentContext) {
    ...
    const allPriorityIssues = results.flatMap(r =>
      r.agent_context?.priority_issues?.map((i: any) => i.field) || []
    );
    ...
    if (urgentProducts.length > 0) {
      ...
      urgentProducts.slice(0, 5).forEach((p: any) => {
        result += `**${p.productId}**\n`;
